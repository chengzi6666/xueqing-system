...
async function loadVersionHistory() {
    try {
        ...
        if (response.ok) {
            ...
            console.log('版本历史加载成功:', versionHistory);
        }
    } catch (error) {
...
function createOverallReportCard(cardData) {
    ...
    if (lessons) {
        const images = [];
        for (let i = 1; i <= 5; i++) {
            const lessonData = lessons[i.toString()];
            
            if (lessonData && lessonData.image) {
                images.push({
                    src: `images/${lessonData.image}`,
                    alt: lessonData.title || `第${i}讲`,
                    lesson: i
                });
            } else {
                // 如果没有图片数据，创建一个默认的
                const defaultImageName = currentSubject === 'math' 
                    ? `数学-${currentLevel}-第${i}讲.png`
                    : `语文-${currentLevel}-第${i}讲.png`;
                images.push({
                    src: `images/${defaultImageName}`,
                    alt: `第${i}讲`,
                    lesson: i
                });
            }
